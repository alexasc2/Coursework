...
for CRAZY_FOR_LOOP in range(0,5):
    #for increment in range(1,11):
            ...
            weights = {
                ...
                'n1' : tf.Variable(tf.random_normal([n_input, n_layer])), 
                'out' : tf.Variable(tf.random_normal([n_layer, n_out])),
                
                #add another weight and biases for each layer
            }
            biases = {
                #'b1' : tf.Variable(tf.random_normal([n_hidden_1])),
                #'b2' : tf.Variable(tf.random_normal([n_hidden_2])),
                'b3' : tf.Variable(tf.random_normal([n_layer])), 
            ...
            def forward(x):
                ...
                layer3 = tf.nn.relu(tf.add(tf.matmul(x,weights['n1']), biases['b3']))
