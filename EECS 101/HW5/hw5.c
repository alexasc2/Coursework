...
int main( int argc, char** argv )
{
	...
	// voting; voting array
	int				sgm_threshold, hough_threshold, voting[THETASIZE][RHOSIZE];
	FILE*			fp;
	...
	for(i = 1; i < ROWS - 1; i++){
		for(j = 1; j < COLUMNS - 1; j++){
			...
			sgmimage[i][j] = sqr(dedx) + sqr(dedy);
	...
	for(i = 1; i < ROWS - 1; i++){
		for(j = 1; j < COLUMNS - 1; j++){
			image[i][j] = 255*((float)sgmimage[i][j])/sgm;
		}
	}
