...
void fifo_schedule(void)
{
	//Evaluates incoming tasks from element 0 of readyQue
	while(readyQue.size() != 0){
			pthread_cond_signal(&(cond[readyQue[0]]));
...
void edf_schedule(void)
{
	//Bubble sorts the queue after each task finished to find the earliest deadline
	//Then runs the task queue in a similar manner to fifo_schedule
	while(readyQue.size() != 0){
		for(int i = 0; i < readyQue.size() - 1;i++){
			for(int k = 0; k < readyQue.size() - i - 1; k++){
				if(tcb[readyQue.at(k)].deadline > tcb[readyQue.at(k + 1)].deadline){
					std::swap(readyQue[k],readyQue[k+1]);
				}
			}
		}
		pthread_cond_signal(&(cond[readyQue[0]]));
