  ...
  /* Lucky you, you get to write MPI code */
	...
	for(int i = 0; i < height; i++){
		...
		if(rank == i%size){
			...
			MPI_Gather(&p_img.front(), width, MPI_INT,
			    &joe_img.front()+i*width, width, MPI_INT, ROOT, MPI_COMM_WORLD);
		}
		y += it;
	}
	...
	if(rank == ROOT){
		...
		int count = 0;
	  	...
	  	string name = "mandelbrot_susie_" + to_string(width) + ".png";
